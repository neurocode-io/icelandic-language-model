...
from language_model.NER.types import InputExample, InputFeatures
...
def read_examples_from_file(file_path: str) -> List[InputExample]:
    """
        Converts a file from  CoNLL format to a list of InputExample
        data[2]
    ("test", ['Að', 'sumu', 'leyti', 'líkist', 'Holberg', 'þeirri', 'mynd', 'sem', 'líffræðilega', 'sinnaðir', 'afbrotafræðingar', 'drógu', 'upp', 'af', 'brotamönnum', 'í', 'lok', '19.', 'aldar'], ['O', 'O', 'O', 'O', 'O', 'B-Person', 'O', 'O', 'O', 'O', 'O', 'O', 'O', 'O', 'O', 'O', 'O', 'O', 'B-Date', 'I-Date']
    """
    ...
    with open(file_path, "r", encoding="utf-8") as f:
        for idx, line in enumerate(f):
            if is_sentence_end(line):
                ...
                data.append(InputExample("train", sentences, labels))
                sentences = []
                labels = []
                continue
